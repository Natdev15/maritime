...
async function main() {
  ...
  try {
    ...
    console.log(`📊 Queue length: ${health.data.queue?.length || 0}`);
    console.log(`⏱️  Server uptime: ${((health.data.uptime || 0) / 1000).toFixed(0)}s`);
    console.log(`📈 Total requests: ${health.data.stats?.totalRequests || 0}`);
    console.log(`⚡ Avg response time: ${health.data.stats?.avgResponseTime || 0}ms\n`);
  } catch (error) {
  ...
  try {
    ...
    if (serverStats) {
      console.log(`📦 Total records: ${serverStats.database?.totalRecords || 0}`);
      console.log(`🏷️  Unique containers: ${serverStats.database?.uniqueContainers || 0}`);
      console.log(`📊 Avg row size: ${tester.formatBytes(serverStats.database?.avgRowSize || 0)}`);
      console.log(`💾 Total data size: ${serverStats.database?.totalSizeMB || 0} MB`);
      console.log(`⚡ Avg processing time: ${serverStats.database?.avgProcessingTime || 0}ms`);
      console.log(`📥 Queue length: ${serverStats.queue?.length || 0}`);
      console.log(`🔄 Is processing: ${serverStats.queue?.isProcessing || false}`);
      console.log(`🎯 Success rate: ${serverStats.server?.successRate || 0}%`);
    }
