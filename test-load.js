...
// node test-load.js bulk --total=100
...
class LoadTester {
  ...
  generateContainerData(containerId, recordIndex = 0) {
    ...
    // Return in the "con" format expected by CBOR Master (no headers)
    const maritimeData = {
    ...
    return {
      con: maritimeData,
      metadata: {
        deviceId: `ESP32_MARITIME_${String(containerId).padStart(3, '0')}`,
  ...
  // Send individual container to CBOR Master (no headers - Slave will add them)
  async sendSingle(container) {
    try {
      const response = await axios.post(`${this.serverUrl}/api/container`, container, {
        headers: {
          'Content-Type': 'application/json',
          'Device-ID': container.metadata?.deviceId || 'ESP32_MARITIME_001',
          'Network-Type': container.metadata?.networkType || 'cellular'
        },
        timeout: 5000
      });
  ...
  // Send a batch of containers (using individual sends to Master)
  async sendBatch(containers) {
    try {
      ...
      for (const container of containers) {
        const result = await this.sendSingle(container);
        if (result.success) {
          processed++;
        } else {
          errors++;
  ...
  async runIndividualLoadTest() {
    ...
    console.log('🔄 Starting Individual CBOR Load Test (Rate Controlled)');
    ...
    console.log(`🗜️  Compression: CBOR (Master → Slave → Mobius)`);
    ...
    for (let i = 0; i < allRecords.length; i++) {
      ...
      try {
        ...
        if (result.success) {
          ...
          // Collect CBOR compression stats
          if (result.data.originalSize && result.data.compressedSize) {
            stats.compressionStats.totalOriginalBytes += result.data.originalSize;
            stats.compressionStats.totalCompressedBytes += result.data.compressedSize;
          }
        } else {
          stats.totalErrors++;
        }
    ...
    if (stats.compressionStats.totalOriginalBytes > 0) {
      stats.compressionStats.compressionRatio = Math.round(
        ((stats.compressionStats.totalOriginalBytes - stats.compressionStats.totalCompressedBytes) / 
         stats.compressionStats.totalOriginalBytes) * 100
      );
    }
  ...
  printReport(stats, testType = 'Individual') {
    ...
    console.log('\n' + '='.repeat(70));
    console.log(`📋 ${testType} CBOR Load Test Results`);
    ...
    if (stats.compressionStats && stats.compressionStats.totalOriginalBytes > 0) {
      console.log('─'.repeat(70));
      console.log('🗜️  CBOR Compression Statistics:');
      console.log(`   Total Original: ${(stats.compressionStats.totalOriginalBytes / 1024).toFixed(2)} KB`);
      console.log(`   Total Compressed: ${(stats.compressionStats.totalCompressedBytes / 1024).toFixed(2)} KB`);
      console.log(`   Average Compression: ${stats.compressionStats.compressionRatio}% saved`);
      console.log(`   Bandwidth Saved: ${((stats.compressionStats.totalOriginalBytes - stats.compressionStats.totalCompressedBytes) / 1024).toFixed(2)} KB`);
    }
...
async function main() {
  ...
  console.log('🚢 Maritime Container Load Tester - CBOR Pipeline');
  console.log('='.repeat(50));
  ...
  console.log(`🗜️  Pipeline: test-load → Master (CBOR) → Slave → Mobius`);
  console.log(`📡 Master URL: ${tester.serverUrl}\n`);

  // Check if server is running
  try {
    const health = await axios.get(`${tester.serverUrl}/api/health`);
    console.log('✅ Master node is running');
    console.log(`📊 Service: ${health.data.service || 'maritime-master-node'}`);
    console.log(`🔧 Mode: ${health.data.mode || 'master'}`);
    console.log(`⏱️  Server uptime: ${((health.data.uptime || 0) / 1000).toFixed(0)}s\n`);
  } catch (error) {
    console.error('❌ Master node not reachable. Please start the server first.');
    console.error('💡 Run: docker-compose up -d maritime-master');
    process.exit(1);
  }
  ...
  try {
    ...
    if (testType === 'individual') {
      ...
      console.log('🔄 Starting Bulk CBOR Load Test');
      console.log('─'.repeat(50));
    ...
    if (serverStats) {
      console.log(`🔧 Service: ${serverStats.service || 'maritime-compression-server'}`);
      console.log(`📡 Mode: ${serverStats.mode || 'master'}`);
      console.log(`💾 Memory Usage: ${Math.round(serverStats.memoryUsage?.heapUsed / 1024 / 1024)}MB`);
      console.log(`⏱️  Uptime: ${serverStats.uptime?.toFixed(0)}s`);
    }
...
if (require.main === module) {
  ...
  console.log('  bulk       - Send containers in batches');
  ...
  console.log('  node test-load.js bulk --total=100 --records=100');
  console.log('  node test-load.js individual --total=100');
  console.log('  node test-load.js bulk --total=100 --records=50 --rate=200');
  console.log('  node test-load.js individual --total=100 --records=100 --rate=10000');
  console.log('');
  console.log('Pipeline: test-load → Master (CBOR compress) → Slave (decompress + headers) → Mobius');
  console.log('');
